...
//   fetch(`http://192.168.1.9:8000/api/details/${encodeURIComponent(email)}/`)
//     .then((res) => res.json())
//     .then((data) => {
...
//      // 🔹 Fetch Tenants
//   fetch(`http://192.168.1.9:8000/api/getbeds/${encodeURIComponent(email)}/`)
//   .then((res) => res.json())
//   .then((data) => {
//     console.log("Tenants API:", data);

//     const formattedTenants = {};
//     const counts = {};

//     data.data.forEach((t) => {
//       const floorLabel = `Floor ${t.floor}`;
//       const roomLabel = String(t.roomno);
//       const key = `${floorLabel}-${roomLabel}`;

//       if (!formattedTenants[key]) {
//         formattedTenants[key] = [];
//       }

//       formattedTenants[key].push({
//         name: t.name,
//         phone: t.phone,
//         email: t.email,
//         bed: t.bed,
...
//     setBedCounts(counts);
//   })
//   .catch((err) => console.log("Tenant Fetch Error:", err));  

// fetch(`http://192.168.1.9:8000/api/getapartmentbeds/${encodeURIComponent(email)}/`)
//   .then((res) => res.json())
//   .then((data) => {
//     console.log("Apartments API:", data);r

//     const formattedApartments = {};
//     const counts = {};

//     data.data.forEach((a) => {
//       const floorLabel = `Floor ${a.floor}`;
//       const roomLabel = String(a.flatno); // ✅ use flatno (NOT roomno)
//       const key = `${floorLabel}-${roomLabel}`;

//       if (!formattedApartments[key]) {
//         formattedApartments[key] = [];
//       }

//       formattedApartments[key].push({
//         name: a.name,
//         phone: a.phone,
//         email: a.email,
//         rent: a.rent,
...
//     setApartments(formattedApartments);       // ✅ new state
//     setApartmentCounts(counts);               // ✅ new state
//   })
//   .catch((err) => console.log("Apartment Fetch Error:", err));
...
//     try {
//       const detailsRes = await fetch(`http://192.168.1.9:8000/api/details/${encodeURIComponent(email)}/`);
//       const detailsData = await detailsRes.json();
//       setResponseData(detailsData);

//       if (detailsData.property_type === "hostel") {
//         const hostelRes = await fetch(`http://192.168.1.9:8000/api/getbeds/${encodeURIComponent(email)}/`);
...
//           `http://192.168.1.9:8000/api/getapartmentbeds/${encodeURIComponent(email)}/`);
  ...
  const fetchData = async () => {
    try {
      const detailsRes = await fetch(
        `http://192.168.1.9:8000/api/details/${encodeURIComponent(email)}/`
      );
      const detailsData = await detailsRes.json();
      setResponseData(detailsData);
      ...
      if (detailsData.property_type === "hostel") {
        const hostelRes = await fetch(
          `http://192.168.1.9:8000/api/getbeds/${encodeURIComponent(email)}/`
        );
        const hostelData = await hostelRes.json();
        ...
        const commercialRes = await fetch(
          `http://192.168.1.9:8000/api/getcommercialbeds/${encodeURIComponent(email)}/`
        );
...
// // formData.append("owner_email", ownerEmail); // ✅ correct key
// console.log("Selected Floor:", selectedFloor);
...
//       "http://192.168.1.9:8000/api/tenentbeds/",
//       {
//         method: "POST",
//         body: formData,
//       }
...
//       "http://192.168.1.9:8000/api/apartmentbeds/",
//        {
//         method: "POST",
//         body: formData,
//       }
...
const saveTenant = async () => {
  ...
  formData.append("sectionNo", selectedRoom); // ✅ correct
    ...
    if (stayType === "hostel") {
      url = "http://192.168.1.9:8000/api/tenentbeds/";
    } 
    else if (stayType === "apartment") {
      
      url = "http://192.168.1.9:8000/api/apartmentbeds/";
    } 
    else if (stayType === "commercial") {
      url = "http://192.168.1.9:8000/api/commercialbeds/"; // or your commercial API
    }
    ...
    addTenant(); // or replace with fetch if needed
...
<View style={styles.controlsRow}>
  ...
  {stayType === "commercial" && selectedRoom}
</Text>
                  <TouchableOpacity
  ...
  (stayType === "hostel"
    ? tenants
    : apartments)[`${selectedFloor}-${selectedRoom}`] ?? []
).map(
                      (t, idx) => (
                ...
                <View style={styles.modalRow}>
  <Text style={styles.modalLabel}>Owner Email</Text>
  <TextInput
    value={ownerEmail || ""}
...
<View style={styles.modalRow}>
  ...
  <TextInput
    value={selectedRoom ? String(selectedRoom).replace("Section ", "") : ""}
    editable={false}
    style={[styles.input, styles.disabledInput]}
  />
</View>
...
//     fetch(`http://192.168.1.20:8000/api/details/${encodeURIComponent(email)}/`)
//       .then((res) => res.json())
//       .then((data) => {
//         console.log("API Response:", data);
//         setResponseData(data);
...
//       const response = await fetch(
//         "http://192.168.1.20:8000/api/tenentbeds/",
//         {
//           method: "POST",
//           headers: {
